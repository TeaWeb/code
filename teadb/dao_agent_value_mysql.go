...
type MySQLAgentValueDAO struct {
}

// 初始化
func (this *MySQLAgentValueDAO) Init() {
...
// 插入数据
func (this *MySQLAgentValueDAO) Insert(agentId string, value *agents.Value) error {
	if value == nil {
...
// 清除数值
func (this *MySQLAgentValueDAO) ClearItemValues(agentId string, appId string, itemId string, level notices.NoticeLevel) error {
	if len(agentId) == 0 {
...
// 查找最近的一条记录
func (this *MySQLAgentValueDAO) FindLatestItemValue(agentId string, appId string, itemId string) (*agents.Value, error) {
	query := NewQuery(this.TableName(agentId)).
		Attr("itemId", itemId).
		Node().
		Desc("createdAt")
...
// 查找最近的一条非错误的记录
func (this *MySQLAgentValueDAO) FindLatestItemValueNoError(agentId string, appId string, itemId string) (*agents.Value, error) {
	query := NewQuery(this.TableName(agentId)).
		Attr("itemId", itemId).
		Attr("error", "").
		Node().
...
// 取得最近的数值记录
func (this *MySQLAgentValueDAO) FindLatestItemValues(agentId string, appId string, itemId string, noticeLevel notices.NoticeLevel, lastId string, size int) ([]*agents.Value, error) {
	query := NewQuery(this.TableName(agentId))
	if len(appId) > 0 {
		query.Attr("appId", appId)
	}
...
// 列出数值
func (this *MySQLAgentValueDAO) ListItemValues(agentId string, appId string, itemId string, noticeLevel notices.NoticeLevel, lastId string, offset int, size int) ([]*agents.Value, error) {
	query := NewQuery(this.TableName(agentId))
	query.Attr("appId", appId)
	query.Attr("itemId", itemId)
	query.Node()
...
// 分组查询
func (this *MySQLAgentValueDAO) QueryValues(query *Query) ([]*agents.Value, error) {
...
// 根据时间对值进行分组查询
func (this *MySQLAgentValueDAO) GroupValuesByTime(query *Query, timeField string, result map[string]Expr) ([]*agents.Value, error) {
	query.FieldMap(this.mapField)
	...
	result["timeFormat.day"] = "timeFormat_hour"
...
// 删除Agent相关表
func (this *MySQLAgentValueDAO) DropAgentTable(agentId string) error {
	return SharedDB().(*MySQLDriver).DropTable(this.TableName(agentId))
}

func (this *MySQLAgentValueDAO) initTable(table string) {
...
func (this *MySQLAgentValueDAO) mapField(field string) string {
	switch field {
